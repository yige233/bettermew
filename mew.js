class mew {
    constructor(setting) {
        this.config = [];
        this.config_extra = [];
        this.settings = (localStorage.settings) ? Object.assign(setting, JSON.parse(localStorage.settings)) : setting;
        this.only_this_mewer = "";
        this.savsetting();
        this.vsrsioncheck();
    };
    static dom(str) {
    ...
    conf(options) {
        ...
        pushconfig(options, this.config);
        let mode = ["once", "loop"],
            conf_key = ["config", "config_extra", "child_funcs"];
        for (let x in conf_key) {
            if (options[conf_key[x]]) {
                for (let i in options[conf_key[x]]) {
                    pushconfig(Object.assign(options[conf_key[x]][i], { parent: options.id }), this[`${["config_extra"].includes(conf_key[x])?conf_key[x]:"config"}`])
                };
            };
        };
        this[`func_config_${options.id}`] = () => { if (options.func_config) return options.func_config.bind(this)() };
        for (let x in mode) {
            this[`func_${mode[x]}_${options.id}`] = () => {
                let always = (options.always) ? true : false;
                if (!this.settings.mainfunc.includes(options.id)) {
                    if (always) {
                        let list = this.settings.mainfunc;
                ...
                if (options[`func_${mode[x]}`]) options[`func_${mode[x]}`].bind(this)();
                for (let i in options.child_funcs) {
                    if (this.settings.childfunc.includes(options.child_funcs[i].id) && options.child_funcs[i][`func_${mode[x]}`]) {
                        options.child_funcs[i][`func_${mode[x]}`].bind(this)();
                    };
                };
            };
        };
    ...
    savsetting(key, value) {
        if (key) { this.settings[key] = (value) ? value : "" };
    ...
    vsrsioncheck() {
        if (!this.settings.ver || this.settings.ver != mew.data.ver) {
            ...
            console.clear();
            for (let i in mew.data.whatsnew) { console.log(`%c${mew.data.whatsnew[i]}`, "color: rgb(125 125 125);font-size:16px") };
    ...
    basic() {
        ...
        const setting = {
                ...
                for (let i in this.config) {
                    ...
                    let funcpath = (func.parent) ? "childfunc" : "mainfunc",
                        ...
                        <li ${(func.parent)?"class='childoption'":""}>
                            ...
                            <div>
                                <input type="checkbox" id="control_${func.id}" title="${func.short_desc}" ${(this.settings[funcpath].includes(func.id))?"checked":"" } ${(func.parent==null || this.settings.mainfunc.includes(func.parent))?"":"disabled"} class="switcher" style="display: none;">
            ...
            config_item: (item) => {
                if (item.type == "text") {
                    let li = mew.dom(`
                        <li style="flex-wrap: wrap;">
                            ...
                            <textarea class="mytextarea" ${(this.settings.mainfunc.includes(item.parent))?"":"disabled"}>${(item.value)?item.value.bind(this)():""}</textarea>
                        </li>`);
                ...
                <li>
                    <span>${item.long_desc}</span>
                    <span class="value">${this.settings[item.id]}%</span>
                    <div><input type="range" id="control_${item.id}" title="${item.long_desc}${this.settings[item.id]}%" value="${this.settings[item.id]}" max="${item.max}" ${(this.settings.mainfunc.includes(item.parent))?"":"disabled"} min="0" step="1" style="width: 300px;"></div>
                </li>`);
            ...
            page: () => {
                ...
                for (let i in this.config_extra) { ul.append(setting.config_item(this.config_extra[i])) };
    ...
    run() {
        for (let i in this.config) { if (this.config[i].func_once && !this.config[i].parent) { this[`func_once_${this.config[i].id}`]() } };
        var observer = new MutationObserver(() => {
            try {
                this.basic(); //基本函数，加载设置页
                for (let i in this.config) { if (this.config[i].func_loop && !this.config[i].parent) { this[`func_loop_${this.config[i].id}`]() } };
            } catch (err) {
                console.log(err);
                return false;
            };
...
mew.data = {
    ...
    whatsnew: [
        ...
        "更新内容：",
        "1、优化细节；设置图标移动到左下角。",
        "2、鉴于想法长文内链接已较为完善，可点击Url功能不再在长文内生效。",
        "3、“只看Ta”功能又回来了。",
        "4、部分功能细化拆分。"
    ],
...
myMew.conf({
    ...
    func_once: function() { mew.loadcss(mew.data.css_darkmode) },
});
myMew.conf({
    id: "desktop",
    short_desc: "桌面布局更改",
    ...
    func_once: function() { mew.loadcss(mew.data.css_desktop) },
    func_loop: function() {
...
myMew.conf({
    ...
    func_once: function() { mew.loadcss(mew.data.css_thought_in_middle) },
});
myMew.conf({
    id: "img_size",
    ...
    func_once: function() { mew.loadcss(mew.data.css_img_size) },
    func_loop: function() {
        document.body.style.setProperty("--img-width", this.settings.img_width + "%");
        document.body.style.setProperty("--img-left", (100 - this.settings.img_width) / 2 + "%");
    }
...
myMew.conf({
    ...
    func_once: function() { return false },
    child_funcs: [{
            id: "compact_thought_hide_img",
            short_desc: "隐藏图片",
            long_desc: "隐藏左侧想法栏图片。",
            func_once: function() { mew.loadcss(mew.data.css_compact_thought_hide_img) }
        },
        {
            id: "compact_thought_hide_text",
            short_desc: "缩减文字",
            long_desc: "缩减左侧想法栏文字高度。",
            func_once: function() { mew.loadcss(mew.data.css_compact_thought_hide_text) }
        },
        {
            id: "compact_thought_more_node",
            short_desc: "更多据点",
            long_desc: "左侧据点栏可显示四个据点。",
            func_once: function() { mew.loadcss(mew.data.css_compact_thought_more_node) }
        }
    ]
});
myMew.conf({
    ...
    func_once: function() { mew.loadcss(mew.data.css_topic_list) },
});
myMew.conf({
    id: "blacklist",
    short_desc: "黑名单",
    ...
    config_extra: [{
        ...
        value: function() { return this.settings.bl.join("\n") },
        ...
        const search = new class {
            ...
            search(str) {
                ...
                fetch(`https://api.mew.fun/api/v1/nodes/${window.location.pathname.slice(3)}/search-thoughts?keyword=${str}&limit=100`, {
                    ...
                    if (res.entries.length == 100) { this.loadres(mew.data.defaultavatar, "提示", "最多只能展示100条结果哦！") };
                }).catch(err => {
                    document.querySelector("#searchres").innerHTML = "";
                    this.loadres(mew.data.defaultavatar, "提示", err);
                });
            };
            serachpage() {
                let s = mew.dom(`
...
myMew.conf({
    ...
    func_once: function() { mew.loadcss(mew.data.css_text2url) },
    func_loop: function() {
...
myMew.conf({
    ...
    func_once: function() { mew.loadcss(mew.data.css_thought_widget) },
    func_loop: function() {
...
myMew.conf({
    ...
    func_once: function() {
        mew.loadcss(mew.data.css_only_this_mewer);
        ...
        let btn = root[root.length - 1].querySelectorAll("[class^='comments_right-btn__']")[0],
            callback = (e) => {
                if (e.target.innerText == "只看作者") {
                    ...
                    if (!nick) { e.stopPropagation(); return false; } else { this.only_this_mewer = nick };
                };
            };
...
myMew.conf({
    ...
    config_extra: [{
        ...
        value: function() { return this.settings.custom_css },
        save: function(e) { return e.target.value }
    }],
    func_once: function() { mew.loadcss(this.settings.custom_css) }
});
myMew.run();